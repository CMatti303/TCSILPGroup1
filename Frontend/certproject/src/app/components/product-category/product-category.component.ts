...
export class ProductCategoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._productService.getProductsByCategory(this.category_id).subscribe(res=>{this.products=res})
    this.categoryName=this._productService.getCategoryName(this.category_id)
    //this._productService.getCategoryName(this.category_id).subscribe(res=>this.categoryName = res['name'])
  ...
  filterProducts(num1: number, num2: number){
    this._productService.getProductsByCategory(this.category_id).subscribe(res=>{
      this.products=res
      let filteredProducts = this.products.filter(product=>{
        return (product.price>=num1 && product.price<=num2)
      })
